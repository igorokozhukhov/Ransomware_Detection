'''
Это главная функция классификации Portable Executable файлов этой программы.
Для извлечения функций из файлов используется библиотека pefile

В этой программе мы сначала извлекаем признаки, а затем передаем их сохраненной модели
и на основе этих признаков предсказываем, является ли файл вредоносным или нет.
'''
...
# Функция для извлечения ресурсов
...
# Извлечение информации о файле с использованием pefile
def extract_infos(fpath):
    ...
    # Загрузка размера конфигурации
    try:
        res['LoadConfigurationSize'] = pe.DIRECTORY_ENTRY_LOAD_CONFIG.struct.Size
    except AttributeError:
        res['LoadConfigurationSize'] = 0

    # Размер конфигурации версии
    try:
...
if __name__ == '__main__':
    ...
    # Загрузка classifier.pkl и features.pkl
    clf = joblib.load('Classifier/classifier.pkl')
    features = pickle.loads(open(os.path.join('Classifier/features.pkl'), 'rb').read())

    # Извлечение характеристик из указанного в аргументе PE-файла
